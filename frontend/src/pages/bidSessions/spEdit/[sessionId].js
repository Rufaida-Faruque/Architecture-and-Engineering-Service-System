...
//     const response = await axios.post(`http://localhost:5000/api/bidSessions/${sessionId}/bid`, {
//       amount: bidAmount,
...
//     try {
//       setLoading(true);
//       await axios.delete(`http://localhost:5000/api/bidSessions/${sessionId}/bid/${myBid._id}`);
...
//           <button onClick={handleDeleteBid}>Withdraw Your Bid</button>
//         </div>
//       )}
//     </div>
//   );
...
//       const timerEndTime = new Date(session.createdAt).getTime() + session.duration * 60 * 60 * 1000; // duration in hours
...
//           // Optionally trigger session closure on frontend here
...
//           if (res.data.winner) {
//             if (res.data.winner._id === localStorage.getItem("userId")) {
...
//         // Set the winner status message only if the session is closed
//         if (res.data.status === "closed") {
//           if (res.data.winner && res.data.winner._id === localStorage.getItem("userId")) {
//             setMessage("Congratulations! You are the winner");
